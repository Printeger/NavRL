...
def test_sweep_dry_run_jobs_include_a2r5g_mechanism_overrides():
    variants = default_safety_preservation_variants()
    assert variants == default_r5g_dry_run_variants()
    jobs = build_jobs(
        python_exe="python",
        variants=variants,
        seeds=[0],
    ...
    expected_variant_overrides = [
        (
            "r5g_smooth025",
            ("algo.instinctRL.governor.smoothing_tau=0.25",),
        ),
        (
            "r5g_smooth040",
            ("algo.instinctRL.governor.smoothing_tau=0.40",),
        ),
        (
            "r5g_anchor_huber050",
            ("instinctRL.anchor.huber_delta=0.50",),
        ),
        (
            "r5g_smooth025_anchor_huber050",
            (
                "algo.instinctRL.governor.smoothing_tau=0.25",
                "instinctRL.anchor.huber_delta=0.50",
            ),
        ...
        (
            ...
            (
                ...
                "instinctRL.ics.downward_ray_min_z=0.10",
        ...
        (
            ...
            (
                "instinctRL.ics.downward_attenuation_enabled=true",
                "instinctRL.ics.downward_ray_min_z=0.05",
                "instinctRL.ics.downward_clearance_margin=0.0",
            ),
        ),
    ]
    expected_names = [name for name, _ in expected_variant_overrides]

    assert [variant.name for variant in variants] == expected_names
    assert [
        (variant.name, variant.overrides) for variant in variants
    ] == [
        (name, base_overrides + overrides)
        for name, overrides in expected_variant_overrides
    ]
    ...
    assert "algo.instinctRL.governor.tracking_vcorr_z_sign_gate_enabled=true" in command_text
    assert "algo.instinctRL.governor.tracking_vcorr_z_gate_eps=0.001" in command_text
    assert "algo.instinctRL.governor.tracking_vcorr_z_opposing_gain=1.0" in command_text
    assert "algo.instinctRL.governor.tracking_vcorr_z_reinforcing_gain=0.50" in command_text
    assert "instinctRL.eval.suite=short_diagnostic" in " ".join(jobs[0].eval_command)

    base_override_pairs = dict(
        override.split("=", 1) for override in base_overrides
    )
    jobs_by_variant = {job.variant: job for job in jobs}
    for variant_name, expected_overrides in expected_variant_overrides:
        ...
        for command in (job.train_command, job.eval_command):
            for key, expected_value in base_override_pairs.items():
                assert _last_override(command, key) == expected_value
                assert f"{key}={expected_value}" in command
    ...
    for job in jobs:
        ...
        assert job.variant.startswith("r5g_")
        assert job.checkpoint_path is None
        assert job.gate_report is None
        ...
        assert "algo.instinctRL.governor.null_vcorr_axis_split_enabled=true" not in command
        assert "algo.instinctRL.governor.null_vcorr_xy_gate_min=0.50" not in command
        assert "algo.instinctRL.governor.null_vcorr_xy_gate_min=0.75" not in command
        assert "algo.instinctRL.governor.null_vcorr_z_gate_min=0.0" not in command
        ...
        assert "algo.instinctRL.governor.tracking_vcorr_z_opposing_gain=0.50" not in command
        assert "algo.instinctRL.governor.tracking_vcorr_z_reinforcing_gain=1.0" not in command
        assert "instinctRL.reward.horizontal_preservation_weight=0.5" not in command
        assert "instinctRL.reward.vertical_preservation_weight=1.0" not in command
        assert "instinctRL.ics.downward_ray_min_z=0.25" not in command
        ...
        assert any(
            arg.startswith("wandb.name=instinctrl_a2r5g_sweep_")
            for arg in job.train_command
        )

    sweep_source = open(SWEEP_PATH, encoding="utf-8").read()
    assert 'parser.add_argument("--tag", default="a2r5g_sweep")' in sweep_source
    assert "a2r5f_sweep" not in sweep_source
