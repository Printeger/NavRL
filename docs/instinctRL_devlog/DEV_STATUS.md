...
- The only permitted disabled attempt is `tests/artifacts/r5j_default_equivalence/20260714_234801/attempts/20260716T074648884514Z-0a6a2be/`. Its checkpoint SHA, seed, legacy argv, branch/commit, cwd, and unique result path passed before CUDA preflight. `nvidia-smi` exited 9 (NVIDIA-driver communication failure) and `torch.cuda.is_available() == false`; eval did not start, no replay JSON exists, and both wrapper record and comparison are `HOLD`.
