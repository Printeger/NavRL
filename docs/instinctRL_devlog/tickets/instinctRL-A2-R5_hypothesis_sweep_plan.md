...
**Status**: A2-R5 is `blocked / stop sweeps`, not ready for training; R5J default-off implementation/provenance repair is complete, its sole disabled replay is consumed, and shutdown integrity remains `HOLD`
**Created**: 2026-07-12  
...
The R5G argv, strict comparator, and wrapper are stored in `docs/instinctRL_devlog/tests/artifacts/r5j_default_equivalence/20260714_234801/`. Before an attempt directory exists, the repaired wrapper records porcelain-v1 status, `worktree_clean`, and verified `source_commit`; only a clean tree with `source_commit == commit == current HEAD` can reach CUDA/eval. A dirty or unresolvable tree receives a provenance-only `HOLD`; any preflight, eval, malformed JSON, comparator, or artifact-write exception is also fail-closed with best-effort parseable artifacts. Provenance repair `8298a7d256bec6a82dee49d9af41a87628135ed6` and synchronization `c2e8367` are pushed on `origin/a2-r5j-default-off-residual`; their historical verification passed pycompile (exit `0`), targeted replay coverage (`19 passed, 1 warning`), the full suite (`163 passed, 13 warnings`), and `git diff --check` (exit `0`). Current shutdown-integrity verification passed pycompile, targeted replay coverage (`22 passed`), full regression (`166 passed, 12 warnings`), and `git diff --check`, without GPU execution. The old `attempts/20260716T074648884514Z-0a6a2be/` record remains historical dirty-worktree/preflight-only `HOLD` and did not consume the then-future replay. The later clean attempt `20260716T161710730878Z-5c9ab7a` consumed that replay: exact data-equivalence comparison returned `GO (design only)`, but stderr contains `Fatal Python error: Segmentation fault` during shutdown despite process exit `0`. The wrapper now detects such explicit fatal markers in either captured stream, stores stream/code/matched-text evidence, and forces a parseable `HOLD`; ordinary warnings and handbook termination metrics do not match. The immutable trace shows `SimulationApp.close()` and W&B's asyncio manager but does not prove causation, so `eval.py` is not changed and no train-only exit workaround is copied. Sweeps remain stopped; do not merge main or evaluate enabled R5J. The only possible next runtime action is separately authorized checkpoint-free GPU shutdown-lifecycle validation. Evidence remains limited by missing contact-body identity, surface normals, measured deceleration, and final safety-fix proof.
